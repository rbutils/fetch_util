  function twitterContent(metadata, pageText) {
    ...
    if (!isStatusPage) {
      ...
      for (var ti = 0; ti < tweetEls.length; ti++) {
      ...
      if (!displayName && !bio && !tweets.length) return null;
    ...
    ["reply", "retweet", "like", "bookmark"].forEach(function(action) {
      ...
      if (btn) {
        ...
        if (countMatch && parseInt(countMatch[1].replace(/,/g, ""), 10) > 0) {
          ...
          if (action === "reply") focalReplyCount = parseInt(countMatch[1].replace(/,/g, ""), 10);
        }
      }
    });
    ...
    var replies = [];
    for (var ri = 1; ri < allTweets.length; ri++) {
    ...
    var statusContent = articleContentFromParts({
    ...
    statusContent.contentType = "social";
    statusContent.socialKind = replies.length ? "thread" : "post";
    statusContent.platform = "X";
    statusContent.handle = focalHandle || null;
    statusContent.replyCount = focalReplyCount;
    return statusContent;
  }

  function registerTwitterProfiles() {

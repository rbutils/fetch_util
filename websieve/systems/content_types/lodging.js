  ...
  function lodgingAmenityItems(root) {
    ...
    function amenityGarbageText(text) {
      return /^(?:show|see|view) (?:more|all)|^(?:read|learn) more|^(?:book|reserve) now|^(?:sign|log) in|^(?:check[- ]in|check[- ]out) policy|property policies$/i.test(text);
    }
  ...
  function lodgingContent(metadata) {
    ...
    return {
      ...
      byline: null,
      excerpt: description ? normalizeText(description) : amenities.join(", "),
      siteName: metadata.siteName || location.hostname,
      publishedTime: null,

  ...
  function mediaWikiListPage(metadata) {
    ...
    var title = normalizeText(mediaWikiTitle(metadata));
  ...
  function mediaWikiTitle(metadata) {
    var heading = document.querySelector("#firstHeading, .firstHeading");
    if (heading) {
      var clone = cleanClone(heading);
      mediaWikiRewriteRoot(clone);
      var title = normalizeText(clone.textContent || "");
      if (title) return title;
    }

    return normalizeText((metadata && metadata.title) || document.title).replace(/\s*[-|–]\s*MediaWiki.*$/i, "");
  }

  function mediaWikiContent(metadata) {
    if (!mediaWikiPage()) return null;

    var title = mediaWikiTitle(metadata);
    ...
    var markdown = mediaWikiListPage(metadata) ? markdownFor(root.innerHTML) : normalizeText(root.textContent || "");
    ...
    return {
      ...
      singleTopicPage: !mediaWikiListPage(metadata),
      hostAware: true
    };
  }

  ...
  function sectionHeadingOwnedByRecord(region, options) {
    var heading = sectionHeadingNode(region, options);
    return !!(heading && closestGenericListFieldCard(heading) && !sectionRecordCollectionFallback(region, options));
  }

  function sectionRegionMarkdown(region) {
    var heading = region.headingNode;
    var links = sectionHeadingLinks(region.node, heading);
    if (region.recordCollectionFallback) {
      var destination = links.length === 1 ? sectionRecordDestination(links[0]) : null;
      return destination && destination.url ? "[" + region.label + "](" + destination.url + ")" : region.label;
    }
  ...
  function sectionRegions(root, options) {
    options = Object.assign({}, options, {
      listContext: listPageContext(),
      recordSectionFallbacks: new WeakMap()
    });
    ...
    candidates.forEach(function(region) {
      ...
      regions.push({
        ...
        recordCollectionFallback: sectionRecordCollectionFallback(region, options)
      });
    });

  function homepageLeadPreservesDestinations(lead, result) {
    if (!lead || !result || !Array.isArray(lead.items) || !Array.isArray(result.items)) return false;
    var destinations = new Set(result.items.map(function(item) {
      var url = materializedHttpUrl(item && item.url);
      return url;
    }).filter(Boolean));
    return lead.items.every(function(item) {
      var url = materializedHttpUrl(item && item.url);
      return !!(url && destinations.has(url));
    });
  }

  function exactHomepageLeadSupplement(lead, content) {
    ...
    for (var additionIndex = 0; additionIndex < additions.length; additionIndex += 1) {
      ...
      var cardDestinations = cardLinks.map(function(link) {
        return materializedHttpUrl(link.getAttribute("href"));
      });
      if (!cardDestinations.length || cardDestinations.some(function(url) { return !url || url !== addition.url; })) return null;
      items.push(addition);
    }

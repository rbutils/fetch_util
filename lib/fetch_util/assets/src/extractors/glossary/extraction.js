  ...
  function glossaryStructuredContent(metadata) {
    ...
    document.querySelectorAll("p.desc").forEach(function(node) {
      var text = normalizeDefinition(node.textContent || "");
      if (!text || text.length < 12 || utilityHeadingText(text)) return;
    ...
    var definitions = leadDefinitions.concat(pairedDefinitions).concat(manyTexts(["p.desc", ".dtText", ".sense .dtText", ".sense", ".def", ".definition", "[class*='definition']", "li[class*='meaning'] > span", "li[class*='definition'] > span", "li[class*='sense'] > span", "[class*='meaning']", "dd", "[itemprop='description']"]).map(normalizeDefinition)).filter(function(text) {
      return text && text.length >= 12 && !utilityHeadingText(text) && !/dictionary definitions page includes all the possible meanings|how to pronounce|how to say|translation|numerology|citation|top lookups/i.test(text);
    ...
    if (pronunciations.length) sections.push(pronunciations.join("\n"));
    definitions.forEach(function(text) {
      sections.push(text);
    });

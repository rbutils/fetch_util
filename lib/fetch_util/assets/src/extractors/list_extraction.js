  ...
  function genericJobListContent(metadata) {
    ...
    var cards = Array.prototype.slice.call(document.querySelectorAll("[data-testid='slider_container'], [data-test='jobListing'], [data-jobid], [data-testid*='job' i], [class*='job-card' i], [class*='jobCard'], tr[data-id][data-url*='/remote-jobs/'], [data-url*='/remote-jobs/']"));
    var seen = {};
    var items = [];
    ...
    cards.forEach(function(card) {
      ...
      if (!title || title.length < minimumListTitleLength(title) || title.length > 220) return;
      key = title + "|" + url;
      if (seen[key]) return;
      seen[key] = true;
      items.push({ text: title, url: url, detail: cardDetail(card, title) });
    });

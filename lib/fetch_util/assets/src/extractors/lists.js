  ...
  function legalStatuteArticleContent(root, text) {
    ...
    var context = normalizeText([document.title || "", location.pathname || "", text.slice(0, 4000)].join(" ")).toLowerCase();
    var legalTitle = /(?:constitution|constitutional|constitui[cç]?[aã]o|c[oó]digo|codigo|code|statute|act|law|regulation|ordinance|decree|treaty|convention)/i.test(context);
    var provisionMarkers = (text.match(/(?:^|\s)(?:Art\.?|Article|Section|Sec\.?|§)\s*(?:\d+[ºª]?|[IVXLCDM]+)/gi) || []).length;
    var structuralMarkers = (text.match(/\b(?:title|chapter|part|book|t[ií]tulo|cap[ií]tulo|se[cç][aã]o)\s+(?:[IVXLCDM]+|\d+)/gi) || []).length;
    var legalTerms = /(?:federal republic|rep[úu]blica federativa|republica federativa|civil rights|fundamental rights|legal provisions?|official gazette|promulgat|enacted|amended|paragraph|par[áa]grafo|paragrafo|inciso|subsection)/i.test(text);

    if (!legalTitle || provisionMarkers < 8) return false;
    ...
    return structuralMarkers >= 3 || legalTerms || longBlocks >= 8 || text.length >= 20000;
  }

  ...
  function dominantIndexListPage(content) {
    ...
    var cards = root.querySelectorAll("article, li, section, [class*='card'], [class*='item'], [class*='story'], [class*='product'], [class*='tile'], [data-testid*='card'], [data-testid*='product']").length;
    ...
    return headlineLinks >= 6 && cards >= 6 && links >= 8 && (headings >= 4 || linkDensity >= 0.22) || articleFeedLinks >= 6;
  }
  ...
  function queryOrCategoryPage() {
    ...
    return /(?:^|[?&])(q|query|search|searchtext|keyword|k)=/.test(query) ||
      /\/(search|s|shop|browse|category|categories|collections?|catalog|keyword|wholesale|products?)\b/.test(path) ||
      /\b(category|categories|collection|catalog|search results?|shop)\b/i.test(document.title || "");
  }

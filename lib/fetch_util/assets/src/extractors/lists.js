  ...
  function searchItemDetail(node, title) {
    ...
    detail = detail
      ...
      .replace(/\bhttps?:\/\/\S+/gi, "")
      .replace(/\s{2,}/g, " ");
    return normalizeText(detail).slice(0, 220);
  }
  ...
  function isProbablyListPage(content) {
    ...
    function listSignals(root) {
      ...
      var listish = root.querySelectorAll("table, ul, ol, .itemlist, .items, .stories, .posts, .news, .headlines, .feed").length;
      var headings = root.querySelectorAll("h1, h2, h3, h4").length;
      var cards = root.querySelectorAll("article, section, [class*='card'], [class*='item'], [class*='story'], [class*='post'], [class*='news'], [class*='headline'], [class*='feed']").length;
  ...
  function likelyListPath() {
    ...
    if (/\/(search|category|categories|tag|tags|topics?|collections?|archive|archives|latest|headlines|news|notizie|calciomercato|mercato)\/?$/.test(path)) return true;
  ...
  function listLinkCandidate(link, container, context) {
    ...
    if (/^(comments?|discuss|hide|more|abonneren|subscribe|newsletter|login|log in|sign in|register|create account|maak een account|instellingen|settings|account)$/i.test(text)) return null;
    ...
    var detail = normalizeText(((container && container.textContent) || "")).replace(text, "").slice(0, 160).replace(/\s*[|·]\s*/g, " - ");
  ...
  function listChromeNode(node) {
    ...
    if (/(news|headline|story|article|post|feed|stream|content|result|listing|archive|topic|feature)/.test(attrs)) return false;
  ...
  function scoreListContainer(node, context) {
    ...
    var cards = node.querySelectorAll("article, section, [class*='card'], [class*='item'], [class*='story'], [class*='post'], [class*='news'], [class*='headline'], [class*='feed']").length;
  ...
  function listCandidateRoot(metadata) {
    ...
    var selectors = ["main", "[role='main']", ".itemlist", "table", "ol", "ul", ".posts", ".stories", ".items", ".news", ".feed", ".headlines", "[class*='news']", "[class*='feed']", "[class*='headline']", "body"];
    var candidates = [];

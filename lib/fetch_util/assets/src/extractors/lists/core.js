  ...
  function listContent(metadata) {
    ...
    var best = candidates.reduce(function(current, node) {
      ...
      if (!current || (result.sectionCount || 0) > (current.sectionCount || 0)) return result;
      if ((result.sectionCount || 0) === (current.sectionCount || 0) && result.score > current.score) return result;
      return current;

  ...
  function sectionCardCandidate(card) {
    if (card.querySelector("article h1 a[href], article h2 a[href], article h3 a[href], article h4 a[href]")) return null;
    var links = cardOwnedNodes(card, "a[href]");
    var link = cardOwnedNodes(card, "h1 a[href], h2 a[href], h3 a[href], h4 a[href]")[0];
    if (!link) {
  ...
  function listMarkdown(items) {
    var seen = {};
    return items.filter(function(item, index, allItems) {
      if (item.card && item.card.querySelector("article h1 a[href], article h2 a[href], article h3 a[href], article h4 a[href]")) return false;
      if (item.card && allItems.some(function(other, otherIndex) {
        return otherIndex !== index && other.card && item.card.contains(other.card);
      })) return false;
      var key = (item.canonicalKey || listCanonicalKey(item.url)).split("?")[0];

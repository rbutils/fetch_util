  function extractListItems(root) {
    ...
    var itemNodes = Array.prototype.slice.call(root.querySelectorAll(itemSelector)).filter(function(node, index, nodes) {
      return !nodes.some(function(nested) { return nested !== node && node.contains(nested); });
    });
    var seen = {};
    var candidates = [];
  ...
  function extractFallbackHeadlineItems(node) {
    ...
    Array.prototype.forEach.call(node.querySelectorAll(selectors), function(link) {
      ...
      if (container && container.querySelector("article h1 a[href], article h2 a[href], article h3 a[href], article h4 a[href]")) return;

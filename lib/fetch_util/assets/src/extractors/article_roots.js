  ...
  function structuredLegalTextRoot() {
    ...
    var hasStatutoryMarkers = /\b(?:U\.S\.\s+Code|Code|Statute|Act|Section|§)\b/i.test(document.title || "") || /(?:^|\s)(?:§|Section|Sec\.?|Article)\s*\d+/i.test(text);
    var enumeratedClauses = (text.match(/(?:^|\s)\(\d+\)|(?:^|\s)\([a-z]\)/gi) || []).length;

    if (text.length < 250 || !hasLegalNotes || !hasStatutoryMarkers || enumeratedClauses < 2) return null;
    return root;
  }

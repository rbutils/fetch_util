  ...
  function docsArticleContent(metadata, node, options) {
    ...
    if (typeof options.rewriteRoot === "function") options.rewriteRoot(root);

    var markdown = markdownFor(root.innerHTML).trim();
    if (typeof options.postProcessMarkdown === "function") markdown = options.postProcessMarkdown(markdown);

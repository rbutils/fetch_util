  var MARKDOWN_MULTILINGUAL_AD_LABEL_LINE_PATTERN = /^\s*(?:Реклама|Рекламний блок|Рекламний матеріал|Διαφήμιση|Iklan|Quảng cáo|โฆษณา|Sponsorlu|Reklama?|Advertentie|Реклама|Оглас|Publicidade|Anúncio|Annons|Annonce|Reklame|Annonsørinnhold|Kommersielt innhold|Contingut patrocinat)\s*:?\s*$/gmi;
  var MARKDOWN_SOCIAL_APP_CTA_LINE_PATTERN = /^\s*(?:(?:Join|Follow)\s+(?:us\s+)?on\s+(?:Telegram|WhatsApp|Instagram|Facebook|YouTube|Twitter|X)|(?:Join|Follow)\s+(?:our\s+)?(?:Telegram|WhatsApp)\s+(?:Group|Channel)|(?:Download|Get|Install)\s+(?:our|the)\s+(?:App|Mobile App))\s*$/gmi;
  var MARKDOWN_MULTILINGUAL_SOCIAL_CTA_PATTERNS = [
    /^\s*(?:Підписуйтесь на наш|Приєднуйтесь до нас|Підписатися на (?:телеграм|канал)|Читай(?:те)? також у|Ми у (?:телеграмі|фейсбуці|інстаграмі))\b.*$/gmi,
    /^\s*(?:Ακολουθήστε μας|Εγγραφείτε)\b.*$/gmi,
    /^\s*(?:Ikuti kami di|Gabung (?:di )?(?:Telegram|WhatsApp)|Berlangganan)\b.*$/gmi,
    /^\s*(?:Theo dõi chúng tôi)\b.*$/gmi,
    /^\s*(?:Volg ons|Schrijf je in|Abonneer je|Meld je aan voor)\b.*$/gmi,
    /^\s*(?:Пратите нас|Претплатите се|Пријавите се|Pratite nas|Pretplatite se|Prijavite se)\b.*$/gmi,
    /^\s*(?:Ikuti kami|Langgan(?:i)?|Sertai (?:kami|saluran))\b.*$/gmi,
    /^\s*(?:Siga-nos|Inscreva-se|Assine|Acompanhe-nos)\b.*$/gmi,
    /^\s*(?:Sledujte nás|Odebírejte|Přihlaste se k odběru|Připojte se)\b.*$/gmi,
    /^\s*(?:Följ oss|Prenumerera|Registrera dig)\b.*$/gmi,
    /^\s*(?:Følg os|Abonner|Tilmeld dig)\b.*$/gmi,
    /^\s*(?:Følg oss|Abonner på|Meld deg på|Registrer deg)\b.*$/gmi,
    /^\s*(?:Subscriu-te|Fer-se subscriptor|Si ets subscriptor|Suma't a|Quins són els avantatges|Fes-te subscriptor|Fes-te'n subscriptor)\b.*$/gmi,
    /^\s*(?:გამოიწერეთ|შემოგვიერთდით|მოგვყევით)\b.*$/gmi,
    /^\s*(?:වැඩි විස්තර කියවන්න|අපව අනුගමනය කරන්න)\b.*$/gmi,
    /^\s*(?:Bizə qoşulun|Abunə olun|Bizi izləyin)\b.*$/gmi
  ];
  var MARKDOWN_POLISH_PROMO_CTA_PATTERNS = [
    /^\s*(?:Subskrybuj|Zasubskrybuj|Zapisz się na newsletter|Dołącz do nas|Obserwuj nas|Śledź nas)\b.*$/gmi,
    /^\s*(?:Dziękujemy,?\s*że\s*przeczytała?(?:ś|ś\/eś|eś)?)\b.*$/gmi,
    /^\s*(?:Bądź na bieżąco)\b.*$/gmi,
    /^\s*(?:Obserwuj nas w Google)\b.*$/gmi,
    /^\s*(?:Zobacz więcej|Czytaj więcej|Czytaj dalej|Pokaż więcej|Więcej informacji)\s*$/gmi,
    /^\s*(?:Dalszy ciąg (?:artykułu|materiału) (?:pod |poniżej )?(?:materiałem wideo|wideo|galerią|zdjęciem))\b.*$/gmi,
    /^\s*(?:Sprawdź,?\s*gdzie kupisz)\b.*$/gmi,
    /^\s*(?:Zaloguj się|Zarejestruj się|Zaloguj|Załóż konto|Moje konto)\s*$/gmi,
    /^\s*(?:Udostępnij|Skomentuj|Komentarze\s*\d*|Dodaj komentarz|Napisz komentarz)\s*$/gmi,
    /^\s*(?:Materiał (?:sponsorowany|promocyjny|partnerski))\b.*$/gmi,
    /^\s*(?:Autopromocja)\s*$/gmi
  ];
  var MARKDOWN_ACCOUNT_LOGIN_CTA_LINE_PATTERN = /^\s*(?:Met een account kan je|Log in of maak een account|Acesse seus artigos salvos|Uma só conta para todos|Maak een account aan|Cadastre-se|Faça login|Faça seu login|Crie sua conta|Meld je aan|Inloggen|Entrar na sua conta|Sign in to save|Create an account|Log in to continue|Log in|Sign in|Create account|Subscribe to read|Daftar atau masuk|Giriş yap|Logg inn|Opprett konto|For å kunne lagre|Inicia sessió|Crea un compte|Registra't|පිවිසෙන්න|ගිණුමක් සාදන්න|Daxil ol|Hesab yaradın)\b.*$/gmi;
  var MARKDOWN_AD_NETWORK_HOST_PATTERN = /(?:outbrain|taboola|zemanta|revcontent|mgid|content\.ad|plista|ligatus|adblade|nativendo|dianomi|nativo|sharethrough|triplelift)\.(?:com|net|co|io)/;
  var MARKDOWN_AD_NETWORK_LINK_PATTERN = new RegExp("\\[([^\\]]*)\\]\\(https?:\\/\\/(?:[a-z0-9-]+\\.)*" + MARKDOWN_AD_NETWORK_HOST_PATTERN.source + "\\/[^)]*\\)", "gi");
  var MARKDOWN_AD_NETWORK_URL_LINE_PATTERN = new RegExp("^\\s*https?:\\/\\/(?:[a-z0-9-]+\\.)*" + MARKDOWN_AD_NETWORK_HOST_PATTERN.source + "\\b[^\\n]*$", "gmi");
  var MARKDOWN_TRAILING_PROMO_MARKERS = [
    /Was (?:this )?(?:summary |article |page )?helpful\?\s+Thank you\s+(?:for\s+)?feedback/i,
    /Trusted by Consumers\.\s+Recognized by AI\./i,
    /(?:^|\n|\s{2,})(?:AI[-\s]generated summary|AI summary|Related content|Related articles|You May Also Like|Need (?:to )?Find (?:an? )?Attorney\?|For Legal Professionals|Get updates|Subscribe to read|Create account|Sign in)\b/i
  ];

  function applyMarkdownPatternList(text, patterns) {
    var result = text;
  ...
  function cleanupMarkdownNoise(markdown) {
    var result = (markdown || "")
      ...
      .replace(MARKDOWN_MULTILINGUAL_AD_LABEL_LINE_PATTERN, "")
      ...
      .replace(MARKDOWN_SOCIAL_APP_CTA_LINE_PATTERN, "")
      ...
      .replace(MARKDOWN_ACCOUNT_LOGIN_CTA_LINE_PATTERN, "")
      ...
      .replace(MARKDOWN_AD_NETWORK_LINK_PATTERN, "")
      // Strip standalone lines that are just ad-network URLs without markdown link syntax
      .replace(MARKDOWN_AD_NETWORK_URL_LINE_PATTERN, "")
      .replace(/\n{3,}/g, "\n\n")
      .trim();

    result = applyMarkdownPatternList(result, MARKDOWN_MULTILINGUAL_SOCIAL_CTA_PATTERNS);
    result = applyMarkdownPatternList(result, MARKDOWN_POLISH_PROMO_CTA_PATTERNS);
  ...
  function stripTrailingArticlePromoTail(markdown) {
    ...
    var markers = MARKDOWN_TRAILING_PROMO_MARKERS;
    var trimAt = -1;
  ...
  function stripLeadingLegalInstitutionalChrome(markdown) {
    var text = markdown || "";
    if (text.length < 400) return text;

    var head = text.slice(0, 5000);
    var chromeBeforeBody = /(?:presid[eê]ncia|casa civil|subchefia|official gazette|government|ministry|department|secretariat|texto compilado|compiled text|\|\s*---\s*\|)/i.test(head);
    var legalCaseChrome = /(?:Home\s*\]\s*\[\s*Databases|Multidatabase Search|World Law|DONATE|depends on users like you|You are here:|New search|Buy [A-Z]+ report|Cite as:)/i.test(head) &&
      /\b(?:judg(?:e)?ment|opinion(?:s)? of the lords|court|appellant|respondent|claimant|defendant|neutral citation|cite as)\b/i.test(head);
    if (!chromeBeforeBody && !legalCaseChrome) return text;

    if (legalCaseChrome) {
      var titleMatch = text.match(/^#\s+[^\n]+\n+/);
      var bodyIndex = head.search(/\n(?:#{1,3}\s+[^\n]*(?:UKHL|UKSC|EWCA|EWHC|UKPC|UKUT|UKFTT|U\.S\.|\bAC\b|\bWLR\b|All\s+ER)\s+\d+[^\n]*|\*\*(?:HOUSE OF LORDS|SUPREME COURT|COURT OF APPEAL|HIGH COURT|OPINIONS? OF THE LORDS)[^\n]*\*\*)/i);
      if (titleMatch && bodyIndex > titleMatch[0].length && bodyIndex < 4500) {
        return (titleMatch[0].trim() + "\n\n" + text.slice(bodyIndex).trim()).trim();
      }
    }

    var preambleMatch = head.match(/(?:\*\*)?\s*(?:PRE[ÂA]MBULO|PREAMBLE)\s*(?:\*\*)?/i);
    if (preambleMatch && preambleMatch.index < 3000) return text.slice(preambleMatch.index).trim();

    var articleMatch = head.match(/(?:^|\s)(?:Art\.?|Article|Section|Sec\.?|§)\s*(?:1|I)\b/i);
    if (articleMatch && articleMatch.index < 3000) return text.slice(articleMatch.index).trim();

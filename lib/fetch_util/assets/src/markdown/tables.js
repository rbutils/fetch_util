  ...
  function tableCellLabelKey(text) {
  ...
  function leadingBlockElement(node) {
    ...
    for (var i = 0; i < node.children.length; i += 1) {
      var child = leadingBlockElement(node.children[i]);
      if (child) return child;
    }
    return null;
  }

  function removeDuplicatedLeadingCellBlocks(cell) {
    var clone = cell.cloneNode(true);
    var children = Array.prototype.slice.call(clone.children);
    for (var i = 1; i < children.length; i += 1) {
      var previous = leadingBlockElement(children[i - 1]);
      var current = leadingBlockElement(children[i]);
  ...
  function tableCellText(cell) {
    ...
    return parts.length ? parts.join(" ") : clone.textContent;
  }

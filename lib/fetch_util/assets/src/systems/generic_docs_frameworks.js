  ...
  function mintlifyDocsContent(metadata, info) {
    ...
    return docsContentBySelectors(metadata, [
      ...
      fallbackTitle: function(metadata) { return (metadata.title || document.title).replace(/\s*[|\-]\s*[^|\-]*$/, ""); },
      rewriteRoot: function(root) {
        root.querySelectorAll("nav, aside, .menu, .sidebar, footer, .pagination, .table-of-contents, .theme-toggle").forEach(function(el) {
          el.remove();
        });
        removeNodesByText(root, "a, button, div, p, span", /^(copy page|copy for llm|view as markdown|ask ai|on this page|table of contents|search|skip to content|skip to main content)$/i);

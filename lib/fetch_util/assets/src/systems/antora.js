  function antoraDocsContent(metadata, info) {
    ...
    if (landing) {
      ...
      var sections = manyTexts(["article.doc h2", "article.doc h3"], 8).filter(function(text) {
      ...
      article.querySelectorAll("a.homepage-link[href], a.homepage-link-primary[href], a.homepage-link-secondary[href]").forEach(function(link) {
        ...
        if (sibling && /^(P|DIV)$/i.test(sibling.tagName || "")) detail = normalizeText(sibling.textContent);
        if (!detail) {
          var container = link.closest("section, div, li, article") || link.parentElement;
          detail = firstRootText(container, ["p"]);
      ...
      if (sections.length) parts.push(sections.slice(0, 6).map(function(text) { return "- " + text; }).join("\n"));

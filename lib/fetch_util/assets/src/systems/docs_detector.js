  function docsSystemInfo(metadata) {
    ...
    var signature = normalizeText([docsHostSignature(metadata), generator, appName].join(" ")).toLowerCase();
    ...
    if (document.querySelector("#dartdoc-main-content, .self-name") && document.querySelector("section.summary dt .name a, section.summary dt a")) {
      return { system: "dartdoc", generator: generator, appName: appName, signature: signature };
    }

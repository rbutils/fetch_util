...
function interstitialContent(metadata, pageText) {
  ...
  if (type === "human_verification") {
    ...
    if (notFoundInterstitialPattern().test(description || "")) {
      description = lines.find(function(text) {
        return text !== title && text.length >= 12 && !notFoundInterstitialPattern().test(text);
      }) || description;
    }

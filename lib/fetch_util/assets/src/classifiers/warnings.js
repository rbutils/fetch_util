  ...
  function normalizedArticleSlugMatchesTitle(metadata, content) {
    if (!metadata || !metadata.language) return false;
    if (!hostMatches(/(^|\.)wikipedia\.org$/i) && !/wikipedia/i.test(metadata.siteName || "")) return false;
    ...
    var canonicalSlug = normalizedSlugText(canonicalSegments[canonicalSegments.length - 1] || "");
    var titleSlug = normalizedSlugText((content && content.title) || metadata.title || "");

    if (!pathSlug || !titleSlug) return false;
    if (pathSlug === titleSlug) return true;
    if (canonicalSlug && (canonicalSlug === pathSlug || canonicalSlug === titleSlug)) return true;

    return pathSlug.indexOf(titleSlug) !== -1 || titleSlug.indexOf(pathSlug) !== -1 ||
      (canonicalSlug && (canonicalSlug.indexOf(pathSlug) !== -1 || canonicalSlug.indexOf(titleSlug) !== -1));
  }
  ...
  function strongSingleTopicPage(metadata, content, markdown, body) {
    ...
    var paragraphCount = ((markdown || "").match(/\n\s*\n/g) || []).length;
    var headingCount = ((markdown || "").match(/^#{1,6}\s+/gm) || []).length;
    var articleLikeRoot = !!document.querySelector("article, main, [itemprop='articleBody'], #mw-content-text .mw-parser-output, #bodyContent .mw-parser-output");

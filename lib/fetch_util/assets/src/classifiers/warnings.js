  ...
  function clearArticleStructure(content, markdown, body) {
    if (!content || content.contentType !== "article" || content.docsLike || body.length < 100 || body.length >= 700) return false;

    var html = String(content.html || "");
    if (!/<article\b/i.test(html) && !/<main\b/i.test(html)) return false;
    var context = normalizeText([content.title || "", body.slice(0, 1000)].join(" ")).toLowerCase();
    if (/\blyrics?\b/.test(context) || /\bpoems?\b/.test(context)) return false;

    var paragraphCount = ((markdown || "").match(/\n\s*\n/g) || []).length;
    var headingCount = ((markdown || "").match(/^#{1,6}\s+/gm) || []).length;
    var sentenceCount = (body.match(/[.!?]+/g) || []).length;
    return (paragraphCount >= 1 || headingCount >= 1) && sentenceCount >= 2;
  }

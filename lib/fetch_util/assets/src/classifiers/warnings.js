  ...
  function syndicatedRepostApplies(title, body, page) {
    ...
    if (legalInstrumentContext(title, body)) return false;
  ...
  function suspicionReasons(metadata, content, markdown, pageText, signals) {
    ...
    var extractedNotFoundBody = extractedNotFoundText.length < 1100 && /\b404\b|\boops!\b|page not found|not the web page you are looking for|we.?re sorry, but (?:that |the )?page cannot be found|(?:that |the )?page cannot be found|we can.?t seem to find the page you.?re looking for|the page you (?:requested|were looking for|are looking for) (?:can.?t be found|does(?:n'?t| not) exist)/i.test(extractedNotFoundText);
    ...
    if (!challengeType && !interstitialType && keywords.length >= 2 && !queryListPage && !docsLike) {
      ...
      if (latinRatio > 0.25) {
        ...
        if ((overlap / keywords.length) < overlapThreshold) {
          ...
          if ((titleOverlap / keywords.length) < overlapThreshold && !scientificDatasetRecordUrl(metadata, content, body)) {
    ...
    if (!challengeType && !interstitialType && content && content.contentType !== "search" && !docsLike && !packagePage) {
      ...
      if (format && !(format === "video" && content.hostAware) && !strongSingleTopicPage(metadata, content, markdown, body) && !scientificRecordContext(metadata, content, markdown, body)) reasons.push("multi_topic_page");
    }

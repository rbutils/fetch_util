  ...
  function localizedSlugArticleGuard(metadata, content, body, markdown) {
    ...
    if (!urlsDifferOnlyByQueryOrSlug(metadata)) return false;

    var path = safeDecodeURI(location.pathname || "");
    var articleTextSample = [metadata.title, body.slice(0, 4000)].join(" ");
    return /[^\x00-\x7F]/.test(path) || (/[^\x00-\x7F]/.test(articleTextSample) && articlePathHasSlugOnlyStructure(normalizedUrlPathSegments(parseMismatchUrl(location.href))));
  }

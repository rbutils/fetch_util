  function suspicionReasons(metadata, content, markdown, pageText, signals) {
    ...
    if (!challengeType && !interstitialType && keywords.length >= 2 && !queryListPage && !docsLike && !courtNumberCasePath(safeDecodeURI(location.pathname || "").toLowerCase()) && !legalCaseContext(title, body)) {
      ...
      if (latinRatio > 0.25) {
        ...
        if ((overlap / keywords.length) < overlapThreshold) {
          ...
          if ((titleOverlap / keywords.length) < overlapThreshold && !localizedSlugArticleGuard(metadata, content, body, markdown) && !investorArchivePageGuard(metadata, content, body, markdown) && !scientificDatasetRecordUrl(metadata, content, body) && !patentPublicationIdMatchesContent(combined)) {
    ...
    if (!challengeType && !interstitialType && body.length > 200 && !courtNumberCasePath(safeDecodeURI(location.pathname || "").toLowerCase()) && !legalCaseContext(title, body)) {
      ...
      if (expectedLang && langStopwords[expectedLang] && !bodyMatchesLanguage(expectedLang, body) && !(expectedLang !== urlLanguageHint() && localizedSlugArticleGuard(metadata, content, body, markdown)) && !investorArchivePageGuard(metadata, content, body, markdown) && !normalizedArticleSlugMatchesTitle(metadata, content)) {
        reasons.push("url_content_mismatch");
      }
    }
    ...
    if (!challengeType && !interstitialType && content && content.contentType !== "search" && !content.isolatedLiveblogEntry && !docsLike && !packagePage) {
      ...
      if (format && !(format === "video" && content.hostAware) && !shortNewsArticle && !strongSingleTopicPage(metadata, content, formatMarkdown, body) && !scientificRecordContext(metadata, content, markdown, body) && !legalInstrumentContext(title, body)) reasons.push("multi_topic_page");
    }

  ...
  function suspicionReasons(metadata, content, markdown, pageText, signals) {
    ...
    if (!challengeType && !interstitialType && content && content.contentType !== "search" && content.contentType !== "press_release" && !content.isolatedLiveblogEntry && !docsLike && !packagePage) {
      ...
      if (format && !credibleListFeed && !(format === "video" && content.hostAware) && !shortNewsArticle && !strongSingleTopicPage(metadata, content, formatMarkdown, body) && !scientificRecordContext(metadata, content, markdown, body) && !legalInstrumentContext(title, body)) reasons.push("multi_topic_page");
    }

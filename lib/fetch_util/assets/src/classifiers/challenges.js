  ...
  function consentLikeInterstitial(interstitialType, combined, body, page) {
    ...
    var cookiePattern = /we use cookies|we use cookies and data|accept all cookies|reject all|more options|cookie preferences|let us know your cookie preferences|allow the use of cookies|cookies from other companies|essential cookies|cookie notice|use cookies and similar technologies|we gebruiken cookies|o vašoj privatnosti|пристанак|колачић|подешавања приватности|vi bruker informasjonskapsler|informasjonskapsler|personvern|vi anvander kakor|samtycke|evasteasetukset|kaytamme evasteita|vi bruger cookies|slapukai|slapuku nustatymai|siklapuku iestatijumi|sīkdatnes|sīkdatņu iestatījumi|sütiket|süti beállítások|adatvédelem/i;
    ...
    var bodyCookieLed = /^(we use cookies|allow the use of cookies|cookie preferences|accept all cookies|before you continue|vi bruker informasjonskapsler|vi anvander kakor|evasteasetukset|vi bruger cookies|izmantojam sīkdatnes|sīkdatņu iestatījumi|sütiket)/.test(normalizedBody) || (normalizedBody.indexOf("cookie") >= 0 && normalizedBody.indexOf("cookie") < 140 && normalizedBody.length < 500);
    var pageCookieLed = /^(we use cookies|allow the use of cookies|cookie preferences|accept all cookies|before you continue|vi bruker informasjonskapsler|vi anvander kakor|evasteasetukset|vi bruger cookies|izmantojam sīkdatnes|sīkdatņu iestatījumi|sütiket)/.test(normalizedPage) || (normalizedPage.indexOf("cookie") >= 0 && normalizedPage.indexOf("cookie") < 140 && normalizedBody.length < 260);
  ...
  function consentWallPage(title, page) {
    ...
    return /before you continue|cookie settings|cookie preferences|privacy choices|manage cookie preferences|consent|privacy gate|jouw privacy-instellingen|о вашој приватности|пристанак|dine personverninnstillinger|personverninnstillinger|samtykke|ditt personvern|vi bruker informasjonskapsler|vi anvander kakor|evasteasetukset|cookie-indstillinger|slapuku nustatymai|siklapuku iestatijumi|sīkdatņu iestatījumi|nastaveni cookies|sutikmeny|süti beállítások/.test(normalizedTitle) ||
      /^(before you continue|cookie settings|let us know your cookie preferences|allow the use of cookies|we use cookies|we gebruiken cookies|о вашој приватности|пристанак|vi bruker informasjonskapsler|vi anvander kakor|evasteasetukset|vi bruger cookies|naudojame slapukus|izmantojam siklapukus|izmantojam sīkdatnes|sütiket)/.test(normalizedPage);
  }
  ...
  function interstitialPageType(metadata, pageText) {
    ...
    var notFoundPattern = /\b404\b|page not found|sorry, (?:we )?(?:can.?t|could not) find (?:that |the )?page|the page you (?:requested|were looking for) (?:can.?t be found|does(?:n'?t| not) exist)|this page is no longer available|content no longer available/i;
    ...
    if (notFoundPattern.test(combined)) {
      var titleLooksMissing = notFoundPattern.test(title.toLowerCase());
      var pageStartsMissing = notFoundPattern.test(page.toLowerCase().slice(0, 240));
      if (!/definitions\.net/i.test(title + " " + location.hostname) && (titleLooksMissing || pageStartsMissing || !substantialPublic)) {
        return "not_found";
      }
    }

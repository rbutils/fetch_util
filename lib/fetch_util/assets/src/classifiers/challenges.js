  ...
  function notFoundInterstitialPattern() {
    return /\b404\b|\boops!\b|page not found|not the web page you are looking for|sorry, (?:we )?(?:can.?t|could not) find (?:that |the )?page|we.?re sorry, but that page cannot be found|(?:that |the )?page cannot be found|the page you (?:requested|were looking for|are looking for) (?:can.?t be found|does(?:n'?t| not) exist)|this page is no longer available|content no longer available/i;
  }
  ...
  function interstitialPageType(metadata, pageText) {
    ...
    if (notFoundInterstitialDominates(page, title)) {
      ...
      if (!/definitions\.net/i.test(title + " " + location.hostname) && (titleLooksMissing || (!substantialPublic && pageStartsMissing) || !substantialPublic)) {
        return "not_found";
      }
    }

  ...
  function highwireArticleContent(metadata) {
    var abstractNode = document.querySelector("#abstract[role='doc-abstract'], section[role='doc-abstract']#abstract, section[property='abstract']");
    var body = document.querySelector("#bodymatter[property='articleBody'], section[property='articleBody'][typeof='Text']");
    ...
    var signatureText = normalizeText([
      ...
      metadata && metadata.title,
      metadata && metadata.siteName,
      firstText(["meta[name='citation_publisher']", "meta[name='citation_journal_title']", "meta[property='og:site_name']"]),
      document.body && document.body.textContent
    ].join(" ")).slice(0, 20000);
    if (!/\b(PNAS|Proceedings of the National Academy|HighWire|citation_doi)\b/i.test(signatureText) && !document.querySelector("meta[name='citation_doi']")) return null;
  ...
  function elsevierArticleContent(metadata) {
    ...
    return assembleScholarlyArticle({
      metadata: metadata,
      abstractRoot: abstracts,
      bodyRoot: body,
      titleSelectors: ["h1", "header h1", ".core-container h1"],
      excerpt: firstText(["#author-abstract", "section[property='abstract']"]),
  ...
  function genericScholarlyArticleContent(metadata) {
    var scholarlySignal = firstScholarlyElement([
    ...
    if (!scholarlySignal) return null;

    var abstractNode = firstScholarlyElement([
      "section[property='abstract']",
      "[role='doc-abstract']",
    ...
    var body = firstScholarlyElement([
    ...
    if (!body) {
      ...
      if (jatsSections.length >= 2) body = jatsSections.reduce(function(best, section) {
    ...
    var abstractText = normalizeText(abstractNode.textContent || "");
    var bodyText = normalizeText(body.textContent || "");
    if (abstractText.length < 80 || bodyText.length < 600) return null;
    if (/get full text access\s+log in,? subscribe or purchase/i.test(bodyText)) return null;

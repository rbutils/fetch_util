  ...
  function highwireArticleContent(metadata) {
    ...
    var root = document.createElement("article");
    var title = firstText([".citation__title", ".article-header h1", "h1[property='name']", "h1", "meta[name='citation_title']"]) || metadata.title || document.title;
    title = normalizeText(title || "").replace(/\s*\|\s*PNAS\s*$/i, "");
    if (title) {
      var heading = document.createElement("h1");
      heading.textContent = title;
      root.appendChild(heading);
    ...
    [abstractNode, body].forEach(function(node) {

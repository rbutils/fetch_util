  function leMondeArticleContent(metadata) {
    ...
    if (offered) leMondeNormalizeOfferedPaywallSignals();
  ...
  function leMondeNormalizeOfferedPaywallSignals() {
    removeAll(document, [
      "#js-modal-gifted-url",
      ".article__gift-modal",
      ".paywall",
    ...
    document.querySelectorAll("p, div, span, section, aside").forEach(function(el) {
      var text = normalizeText(el.textContent || "");
      if (text.length > 260) return;
      if (/cet article vous est offert|pour lire gratuitement cet article réservé aux abonnés|article réservé aux abonnés|la suite est réservée à nos abonnés/i.test(text)) el.remove();
    });

    document.querySelectorAll("script[type='application/ld+json']").forEach(function(script) {
      var text = script.textContent || "";
      if (/isAccessibleForFree/i.test(text)) {
        script.textContent = text.replace(/("isAccessibleForFree"\s*:\s*)"?false"?/gi, "$1true");
      }
    });
  ...
  var leMondeBaseHostAwareContent = hostAwareContent;
  hostAwareContent = function(metadata, pageText) {
    return leMondeArticleContent(metadata) || leMondeBaseHostAwareContent(metadata, pageText);
  };

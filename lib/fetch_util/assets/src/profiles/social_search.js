  ...
  function tikTokContent(metadata, pageText) {
    ...
    if (/^\/tag\//.test(location.pathname) || heading || subheading) {
      ...
      if (!description || /make your day/i.test(description)) description = "This TikTok page is partially gated, but the visible tag or page summary can still be extracted.";

      var details = [];
      if (subheading && subheading !== title) details.push(subheading);
      if (/drag the slider to fit the puzzle|log in/i.test(page)) details.push("Access: TikTok login or verification wall");
  ...
  function ebaySearchContent(metadata, pageText) {
    ...
    return articleContentFromParts({
      title: metadata.title || document.title,
      description: metadata.excerpt || "This eBay page is showing a cookie or privacy wall before full results are available.",
      details: ["Access: eBay cookie or privacy wall"],
      siteName: metadata.siteName || "eBay",
      contentType: "article"
    });
  }

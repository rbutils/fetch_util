  function officialStatutePageSignature(doc, metadata) {
    ...
    return /\b(?:code|act|statute|regulation|ordinance)\b/i.test(title) &&
      /\b(?:version information|translation provided by|translations? may not be updated|full citation|federal law gazette|table of contents)\b/i.test(text) &&
  ...
  function structuredLegalProvisionContent(metadata) {
    ...
    var enumeratedClauses = (sourceText.match(/(?:^|\s)\(\d+\)|(?:^|\s)\([a-z]\)/gi) || []).length;
    var provisionSignature = /\b(?:U\.S\.\s+Code|Code|Statute|Act|Section|§)\b/i.test(titleContext) || /(?:^|\s)(?:§|Section|Sec\.?|Article)\s*\d+/i.test(sourceText);

    if (sourceText.length < 250 || !hasSupplementaryNotes || !provisionSignature || enumeratedClauses < 2) return null;

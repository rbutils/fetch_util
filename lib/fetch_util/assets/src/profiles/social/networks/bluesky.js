  function blueskyContent(metadata, pageText) {
    ...
    if (isProfilePage && !isPostPage) {
      ...
      for (var pi = 0; pi < postTextEls.length; pi++) {
    ...
    var postSchema = structuredDataNode("DiscussionForumPosting");
    if (!postSchema) {
      var schemaPage = structuredDataNode("WebPage");
      if (schemaPage && schemaPage.mainEntity && nodeTypes(schemaPage.mainEntity).indexOf("DiscussionForumPosting") !== -1) {
        postSchema = schemaPage.mainEntity;
      }
    }
    var currentPostRoot = null;
    var postRoots = [];
    if (focalHandle) {
      postRoots = document.querySelectorAll('[data-testid^="postThreadItem-by-"]');
      for (var pr = 0; pr < postRoots.length; pr++) {
        if (postRoots[pr].getAttribute("data-testid") === "postThreadItem-by-" + focalHandle.slice(1)) {
          currentPostRoot = postRoots[pr];
          break;
        }
      }
    }

    var schemaMatchesPage = postSchema && postSchema.url && new URL(postSchema.url, location.href).pathname === location.pathname;
    if (!focalPost && !(currentPostRoot && schemaMatchesPage)) return null;

    var focalText = focalPost ? normalizeText(focalPost.textContent) : entityText(postSchema.text);
    ...
    if (!focalAuthor && postSchema && postSchema.author) focalAuthor = entityName(postSchema.author);
    if (!focalAuthor) focalAuthor = (metadata.title || "").replace(/\s*[-–—|].*$/, "").trim();

    var focalFeedItem = focalPost && (focalPost.closest('[data-testid^="feedItem-by-"]') || document.querySelector('[data-testid^="feedItem-by-"]'));
    if (!focalFeedItem) focalFeedItem = currentPostRoot;
    var focalEngagement = [];
    ...
    var replies = [];
    if (currentPostRoot && postRoots.length > 1) {
      for (var ci = 0; ci < postRoots.length; ci++) {
        if (postRoots[ci] === currentPostRoot) continue;
        var currentReply = normalizeText(postRoots[ci].textContent);
        if (currentReply && currentReply.length > 3) replies.push(currentReply);
      }
    } else if (postSchema && Array.isArray(postSchema.comment)) {
      for (var si = 0; si < postSchema.comment.length; si++) {
        var schemaReply = entityText(postSchema.comment[si] && postSchema.comment[si].text);
        if (schemaReply && schemaReply.length > 3) replies.push(schemaReply);
      }
    } else {
      var allPostTexts = document.querySelectorAll('[data-testid="postText"]');
      for (var ri = 1; ri < allPostTexts.length; ri++) {

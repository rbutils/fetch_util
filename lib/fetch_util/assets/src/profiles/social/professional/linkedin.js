  function linkedinContent(metadata, pageText) {
    ...
    if (isCompanyPage) {
      ...
      if (updatesSection) {
        ...
        for (var pi = 0; pi < postBlocks.length; pi++) {
      ...
      if (!aboutText && !posts.length && !followerCount && !industry && !companySize && !headquarters && !specialties) return null;

      var companyContent = articleContentFromParts({
      ...
      companyContent.contentType = "social";
      companyContent.socialKind = "profile";
      companyContent.platform = "LinkedIn";
    ...
    if (activityMatch) {
      ...
      if (artMatches) {
        for (var ai = 0; ai < artMatches.length; ai++) {
    ...
    if (!personTitle && !personFollowers && !personConnections && !personLocation && !personAbout && !articles.length) return null;
    ...
    profileContent.platform = "LinkedIn";

  function threadsContent(metadata, pageText) {
    ...
    var publicRoot = visibleMetaSocialRoot("main, [role='main']");
    ...
    if (!publicRoot || (!displayName && !handle)) {
      return null;
    }

    var rootText = normalizeText(publicRoot.innerText || "");
    if (!rootText || (handle && rootText.indexOf(handle) === -1 && !/\bfollowers?\b/i.test(rootText))) return null;

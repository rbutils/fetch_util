  ...
  function jangDetailArticleContent(metadata) {
    var article = document.querySelector(".story-content, .news-detail, .article-body, div.nn-article-body, .detail_view_content, .description");
  ...
  function jangLiveblogStoryContent(metadata) {
    ...
    return liveblogSingleEntryContent(metadata, {
      selectedEntry: jangLiveblogSelectedStory,
      title: function(md, root, story) { return firstTextIn(story, ["h2"]) || firstText([".time_first_story h1", "h1"]) || md.title; },
      bylineSelector: [".by_author h5"],
      minTextLength: 120,
      rewriteRoot: jangCleanArticleRoot
    });
  }
  ...
  registerHostAwareProfile(true, jangArticleContent);

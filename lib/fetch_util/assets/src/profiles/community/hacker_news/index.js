  function hackerNewsContent(metadata) {
    if (!hostMatches(/^news\.ycombinator\.com$/i)) return null;
  ...
  function hackerNewsFeedContent(metadata) {
    var stories = Array.prototype.slice.call(document.querySelectorAll("table.itemlist tr.athing")).map(hackerNewsStory).filter(Boolean);

  function pikabuContent(metadata) {
    ...
    var result = profileArticleContent(metadata, body, {
      title: pikabuStoryTitle(story, metadata),
      byline: pikabuStoryHandle(story) || metadata.byline,
      minTextLength: 60,
    ...
    result.socialKind = "post";
    result.platform = "Pikabu";
    result.handle = pikabuStoryHandle(story);
    return result;
  }
  ...
  function pikabuStoryBody(story, metadata) {
    ...
    var title = pikabuStoryTitle(story, metadata);
    if (title) {
    ...
    var metaText = normalizeText([
      pikabuStoryHandle(story),
      firstTextFromNode(story, [".story__datetime-link", ".story__datetime"])
    ].filter(Boolean).join(" - "));
    if (metaText) {
  ...
  function pikabuStoryTitle(story, metadata) {
    return firstTextFromNode(story, [".story__title", "h1.title", "h1"]) || (metadata && metadata.title);
  }

  function pikabuStoryHandle(story) {
    return firstTextFromNode(story, [".story__user-link"]);
  }

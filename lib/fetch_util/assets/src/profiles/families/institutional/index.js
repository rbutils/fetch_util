  ...
  function normalizeComponentHub(metadata, root, options) {
    ...
    return institutionalArticleResult(metadata, root, {
      ...
      strip: function(clone) {
        ...
        removeAll(clone, options.chromeSelector || COMMON_INSTITUTIONAL_CHROME_SELECTOR + ", [class*='breadcrumb' i], [class*='social' i], [class*='share' i], [class*='embed' i], [class*='newsletter' i], form");
        if (options.strip) options.strip(clone);
      }
    });
  }

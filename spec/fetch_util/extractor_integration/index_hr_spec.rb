...
RSpec.describe 'FetchUtil Index.hr extractor integration' do
  ...
  it 'extracts Index.hr article bodies without stale_content on recirculated sport articles' do
    ...
    %w[www.index.hr journal.example].each do |host|
      extract_from_url("https://#{host}/sport/clanak/dinamo-vraca-projekt-na-kojemu-je-godinama-zaradjivao-milijune-evo-svih-detalja/2796419.aspx", html) do |payload|
        ...
        expect(payload['byline']).to eq('Tomislav Dragobratović')
        expect(payload['publishedTime']).to eq('2026-05-28')
        ...
        expect(payload['markdown']).to include('### Jerko Leko u intervjuu za Index')

...
RSpec.describe FetchUtil::Browser do
  ...
  it 'does not count hidden consent templates as handled overlays' do
    ...
    expect(page).to have_received(:evaluate) do |script|
      expect(script).to include('!overlayLike || !visible(el)')
  ...
  it 'patches empty userAgentData values to a consistent chrome profile' do
    browser = described_class.new(browser_path: '/usr/bin/chromium', wait: 0)
  ...
  it 'preserves google consent walls instead of auto-accepting them' do
    network = instance_double('FerrumNetwork')
    page = instance_double(Ferrum::Browser)
  ...
  it 'uses a fixed social-login phase pause when wait is zero' do
    browser = described_class.new(browser_path: '/usr/bin/chromium', wait: 0)
  ...
  it 'reuses the shared settling helper for social-login phase pauses when wait is positive' do
  ...
  it 'uses the shared social-login phase pause sequence for instagram stabilization' do
    page = instance_double(Ferrum::Browser)
    browser = described_class.new(browser_path: '/usr/bin/chromium', wait: 0, wait_for_idle: true)
  ...
  it 'uses the shared social-login phase pause sequence for facebook stabilization' do
    page = instance_double(Ferrum::Browser)
    browser = described_class.new(browser_path: '/usr/bin/chromium', wait: 0, wait_for_idle: true)

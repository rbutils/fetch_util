...
RSpec.describe FetchUtil::Fetcher do
  ...
  it 'preserves instagram next redirect targets when the current session is bounced to login' do
    ...
    redirected_payload = payload.merge(
      ...
      'markdown' => '# Cristiano Ronaldo (@cristiano)\n\nThis Instagram page for cristiano requires login before the original content is available.',
  ...
  it 'enriches thin instagram profile fallbacks from observed profile traffic' do
    ...
    instagram_payload = payload.merge(
      ...
      'markdown' => [
        ...
        '- Access: Instagram login wall',
        '- Image: https://cdn.example.test/cristiano.jpg',
        '- 673M Followers, 643 Following, 4,027 Posts.'
      ].join("\n\n"),
      'canonicalUrl' => 'https://www.instagram.com/cristiano/'
    )
    response = instance_double('FerrumResponse', status: 200, body: JSON.generate(
      'data' => {
        'user' => {
          'username' => 'cristiano',
          'full_name' => 'Cristiano Ronaldo',
          'biography' => 'SIUUU. Join me on YouTube.',
          'bio_links' => [
            { 'title' => 'UR Cristiano', 'url' => 'https://youtube.com/@cristiano' },
            { 'title' => 'WHOOP', 'url' => 'https://join.whoop.com/cr7' }
          ],
          'edge_followed_by' => { 'count' => 672_636_856 },
          'edge_follow' => { 'count' => 630 },
          'edge_owner_to_timeline_media' => { 'count' => 4027 },
          'is_verified' => true,
          'category_name' => 'Athlete'
        }
      }
    ))
    exchange = instance_double('FerrumExchange', url: 'https://www.instagram.com/api/v1/users/web_profile_info/?username=cristiano', response: response)
    network = instance_double('FerrumNetwork', traffic: [exchange])

    allow(instagram_page).to receive(:network).and_return(network)
    ...
    expect(result.title).to eq('Cristiano Ronaldo (@cristiano)')
    expect(result.excerpt).to eq('SIUUU. Join me on YouTube.')
    expect(result.markdown).to include('- Verified account')
    expect(result.markdown).to include('- Category: Athlete')
    expect(result.markdown).to include('- 672,636,856 followers, 630 following, 4,027 posts')
    expect(result.markdown).to include('- Image: https://cdn.example.test/cristiano.jpg')
    expect(result.markdown).to include('## Links')
    expect(result.markdown).to include('[UR Cristiano](https://youtube.com/@cristiano)')
    expect(result.markdown).not_to include('Access: Instagram login wall')
  end

  it 'falls back to graphql instagram profile traffic when web_profile_info is unauthorized' do
    instagram_page = instance_double('FerrumPage', current_url: 'https://www.instagram.com/cristiano/')
    instagram_payload = payload.merge(
      'title' => 'Cristiano Ronaldo (@cristiano)',
      'excerpt' => '673M Followers, 643 Following, 4,027 Posts.',
    ...
    graphql_response = instance_double('FerrumResponse', status: 200, body: JSON.generate(
      'data' => {
        'user' => {
          'username' => 'cristiano',
          'full_name' => 'Cristiano Ronaldo',
          ...
          'follower_count' => 672_636_856,
          'following_count' => 630,
          'media_count' => 4027,
          'is_verified' => true,
          'category' => 'Athlete',
          'profile_pic_url' => 'https://cdn.example.test/cristiano.jpg'
        }
      }
    ))
    web_profile_exchange = instance_double('FerrumExchange', url: 'https://www.instagram.com/api/v1/users/web_profile_info/?username=cristiano', 
                                                             response: unauthorized_response)
    graphql_exchange = instance_double('FerrumExchange', url: 'https://www.instagram.com/graphql/query', response: graphql_response)
    network = instance_double('FerrumNetwork', traffic: [web_profile_exchange, graphql_exchange])
    ...
    expect(result.markdown).to include('- 672,636,856 followers, 630 following, 4,027 posts')
    expect(result.markdown).to include('- Image: https://cdn.example.test/cristiano.jpg')
    expect(result.markdown).to include('[Perplexity](http://perplexity.ai/ronaldo)')
    expect(result.markdown).not_to include('Access: Instagram login wall')
  end
